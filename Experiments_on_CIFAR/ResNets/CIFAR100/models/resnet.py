...
class Bottleneck_leaky(nn.Module):
    expansion = 4

    def __init__(self, in_planes, planes, stride=1):
        super(Bottleneck_leaky, self).__init__()
        self.conv1 = nn.Conv2d(in_planes, planes, kernel_size=1, bias=False)
        self.bn1   = nn.BatchNorm2d(planes)
        self.conv2 = nn.Conv2d(planes, planes, kernel_size=3, stride=stride, padding=1, bias=False)
        self.bn2   = nn.BatchNorm2d(planes)
    ...
    def forward(self, x):
        out = F.leaky_relu(self.bn1(self.conv1(x)), negative_slope=0.01)
        out = F.leaky_relu(self.bn2(self.conv2(out)), negative_slope=0.01)
        out = self.bn3(self.conv3(out))
        out += self.shortcut(x)
        out = F.leaky_relu(out, negative_slope=0.01)
        return out
...
class ResNet_cifar_leaky(nn.Module):
    def __init__(self, block, num_blocks, num_classes=100):
        super(ResNet_cifar_leaky, self).__init__()
        self.in_planes = 16
...
class WResNet_cifar(nn.Module):
    def __init__(self, block, num_blocks, k, num_classes=100):
...
def ResNet20_leaky():
    depth = 20
    n = (depth - 2) // 6
    return ResNet_cifar_leaky(BasicBlock_leaky, [n,n,n])
def ResNet20():
    depth = 20
    n = (depth - 2) // 6
