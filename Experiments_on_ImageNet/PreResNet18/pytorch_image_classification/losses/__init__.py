...
class CrossEntropyLoss_act(_WeightedLoss):
    ...
    def forward(self, input, target):
        ...
        #err = torch.log(torch.exp(err)-0.5)#For ResNet18 and  PreResNet18
        #err = torch.log(torch.exp(err)-0.6)#For ResNet34
        #err = (torch.atan(0.5*err))**2 + 0.3*err#For DenseNet121
